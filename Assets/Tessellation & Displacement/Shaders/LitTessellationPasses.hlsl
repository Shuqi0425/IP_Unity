...
// Fragment shader
#if defined(PASS_FORWARD) || defined(PASS_DEPTH_NORMALS)
void TessellatedFragment(
	TessellatedVaryings input
	, out half4 outT0 : SV_Target0
...
half TessellatedFragment(TessellatedVaryings input) : SV_TARGET
#else
...
#endif
{
	...
	#if defined(PASS_FORWARD) || defined(PASS_DEPTH_NORMALS)
	PassFragmentProgram(input.nativeVaryings, outT0);
	#ifdef _WRITE_RENDERING_LAYERS
	PassFragmentProgram(input.nativeVaryings, outT0, outT1);
	#endif
	#else
